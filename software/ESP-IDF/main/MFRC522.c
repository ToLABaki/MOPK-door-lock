/*
* MFRC522.cpp - Library to use ARDUINO RFID MODULE KIT 13.56 MHZ WITH TAGS SPI W AND R BY COOQROBOT.PCD_WriteRegister
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_WriteRegisters(	enum PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_ReadRegisters(	enum PCD_Register reg,	///< The register to read from. One of the PCD_Register enums.
	...
	while (!uidComplete) {
		...
		while (!selectDone) {
			...
			if (currentLevelKnownBits >= 32) { // All UID bits in this Cascade Level are known. This is a SELECT.
				//printf(F("SELECT: currentLevelKnownBits=")); printfln(currentLevelKnownBits, DEC);
				...
				//printf(F("ANTICOLLISION: currentLevelKnownBits=")); printfln(currentLevelKnownBits, DEC);
...
void PICC_DumpDetailsToSerial(Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	// (suggested) PICC type
	enum PICC_Type piccType = PICC_GetType(uid->sak);
	printf("PICC type: %s", PICC_GetTypeName(piccType));
...
void PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
		...
		if (group != 3 && (g[group] == 1 || g[group] == 6)) { // Not a sector trailer, a value block
			...
			printf(" Value=0x %ld", value); 
			printf(" Adr=0x %d", buffer[12]); 
		}
		printf("\n");
	}
...
uint8_t MIFARE_OpenUidBackdoor(uint8_t logErrors) {
	...
	if (received != 1 || response[0] != 0x0A) {
		if (logErrors) {
			printf("Got bad response on backdoor 0x40 command: %d (%d valid bits)\n",response[0], validBits );
		}
		return 0;
	}
	...
	if (received != 1 || response[0] != 0x0A) {
		if (logErrors) {
			printf("Got bad response on backdoor 0x43 command: %d (%d vaild bits)\n",response[0], validBits);
		}
		return 0;
	}
...
//			  PICC_WakeupA(atqa_answer, &atqa_size);
	...
	if (status != STATUS_OK) {
		if (logErrors) {
            printf("MIFARE_Read() failed: %s\nAre you sure your KEY A for sector 0 is 0xFFFFFFFFFFFF?", GetStatusCodeName(status));
		}
		return 0;
	}
