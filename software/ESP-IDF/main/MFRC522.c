...
uint8_t PCD_ReadRegister(
	...
	uint8_t transmit_data[2] = {0x80 | (uint8_t)reg, 0x00};
...
void PCD_ReadNRegister(
    ...
    memcpy(values, &recieve_data[1], count);
}
...
void PCD_Init(MFRC522Ptr_t mfrc) {
    ...
    ret = spi_bus_initialize(VSPI_HOST, &buscfg1, SPI_DMA_DISABLED);
    if(ret != ESP_OK){
        printf("SPI ERR1\n");
        
    }
    ret = spi_bus_add_device(VSPI_HOST, &cfg1, &handle1);
    if(ret != ESP_OK){
        printf("SPI ERR1\n");
...
uint8_t PCD_SelfTest(MFRC522Ptr_t mfrc) {
    ...
    PCD_WriteRegister(mfrc, CommandReg, PCD_SoftReset);
    vTaskDelay(50 /portTICK_PERIOD_MS); //Allow the chip to reset
